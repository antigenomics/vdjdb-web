# Data sources

Every dataset this repository reads or ships, where it came from, and how to regenerate it.

---

## `test/resources/database/` — VDJdb test fixture

**What it is:** a 2,000 row subset of the VDJdb database, checked into the repository.
**Provenance:** *derived*, not experimental — a deterministic subsample of the production database.
**Format:** two tab-separated files, `vdjdb.txt` (27 columns, 2,000 rows) and `vdjdb.meta.txt`
(27 rows, one per data column). The two must stay in lockstep: `VdjdbInstance` reads the meta stream
to build the schema and then adds the data rows against it, so a column added on one side without the
other fails at load.

**Why it exists:** `BaseTestSpecWithApplication` boots a Guice application. Without a local database
`Database` calls `new VdjdbInstance()`, which calls `Util.checkDatabase()` and downloads the whole of
VDJdb over the network before any assertion runs.

**Upstream:** the production database at `vdjdb@100.83.178.73:~/vdjdb_publish/vdjdb-db/`, mounted into
the application container at `/home/vdjdb/vdjdb-db/`.

That file is itself derived from the VDJdb release, most recently
`https://github.com/antigenomics/vdjdb-db/releases/download/2026-06-03/vdjdb-2026-06-03.zip`
(see `latest-version.txt` in the same directory), and a copy of the 2026-06-11 ZENODO build is
mirrored at
`https://huggingface.co/datasets/isalgo/airr_benchmark/tree/main/vdjdb/vdjdb-2026-06-11-ZENODO`
(locally at `~/hf/airr_benchmark/vdjdb/vdjdb-2026-06-11-ZENODO/`).

⚠️ **Neither upstream is the direct parent of the fixture.** The released and mirrored files carry
**22 columns**; production carries **27** — it adds `evidence.validation.same.study`,
`evidence.validation.independent`, `evidence.structure.contacts`, `evidence.structure.quality` and
`evidence.structure.native`.

Three of the five are generated **in this repository**, by `tools/reconcile_structures.py` (2026-07-19),
which defines them precisely:

| Column | Definition |
|---|---|
| `evidence.structure.native` | `meta.structure.id` is a real PDB accession (`1abc` or `pdb_XXXXXXXX`) |
| `evidence.structure.contacts` | the row's `TCR_hash` has a model with `num_contacts > 0` |
| `evidence.structure.quality` | the row's `TCR_hash` has a model with `ranking_confidence >= 0.85` |

The remaining two — `evidence.validation.same.study` and `evidence.validation.independent` — are still
generated by a step I could not locate in this repository or on the server
(`~/data_prep/concat_for_release.ipynb` is the closest candidate and was not confirmed).

**The fixture was taken from production because that is the only place the 27-column form exists.** If
you need to rebuild it from a public source, that missing step has to be identified first — do not
substitute the 22-column release, as the meta file and every `evidence.*` filter expect 27.

**Regenerate:**

```bash
# On the server, or anywhere holding the 27-column vdjdb.txt + matching vdjdb.meta.txt.
SRC=/home/vdjdb/vdjdb_publish/vdjdb-db
TOTAL=$(($(wc -l < "$SRC/vdjdb.txt") - 1))
STEP=$((TOTAL / 2000))

# 1. Uniform spread, header included.
awk -v step="$STEP" 'NR==1 || (NR-1) % step == 0' "$SRC/vdjdb.txt" > vdjdb.txt

# 2. Top up high-confidence rows to 150 - SearchTableSpec pages a "vdjdb.score >= 3" result set at
#    page size 100 and asserts page 0 is full, and step 1 yields only ~89 such rows.
HAVE=$(awk -F'\t' 'NR>1 && $14==3' vdjdb.txt | wc -l)
awk -v step="$STEP" -v need="$((150 - HAVE))" -F'\t' \
  'NR>1 && (NR-1) % step != 0 && $14==3 && n < need { print; n++ }' "$SRC/vdjdb.txt" >> vdjdb.txt

cp "$SRC/vdjdb.meta.txt" vdjdb.meta.txt
```

Every Nth row rather than the head: the source is grouped by reference, so the first 2,000 rows would
be almost entirely one study. Deterministic on purpose — a fixture that resampled per run would make
a test failure impossible to reproduce.

⚠️ **The result is deliberately not distributionally faithful** — step 2 over-represents
`vdjdb.score = 3` roughly fourfold. It is a fixture for exercising code paths, not a sample to measure
anything on. One row appears twice, which is also true in the source.

See `test/resources/database/README.md` for the coverage the current specs depend on.

---

## `application.database.path` — runtime VDJdb

**Production:** `~/vdjdb_publish/vdjdb-db/` on the deployment host, bind-mounted read-only to
`/home/vdjdb/vdjdb-db/` (see `docker-compose.yml`). Alongside `vdjdb.txt`/`vdjdb.meta.txt` it holds
`cluster_members.txt` and `cluster_members_tcremp.txt` (TCRNET and TCREMP motif cluster membership),
`motif_pwms*.txt`, `structures_metadata.tsv`, and `motif-charts/`.

**Development:** `database/` relative to the working directory, per `conf/application.conf`. Absent by
default; the application downloads a release into `~/.vdjdb` when it is missing.

---

## `vdjdb-db/structure/` — TCR–pMHC contact maps

**What it is:** per structure, a 2D contact map plus the data behind it. `Structures` resolves
`<hash>.html` / `<hash>_simplified.html` into `/structure-files/...` URLs; the frontend fetches the
markup and injects it into the structure viewer.

**Provenance:** *derived/computed* — every file is produced from a modelled or native PDB structure.
None of it is measured.

| File | Contents |
|---|---|
| `<hash>.svg` / `<hash>.html` | contact map; matplotlib SVG, embedded in a bare HTML wrapper |
| `<hash>_simplified.svg` / `.html` | reduced variant, used for the stacked overlay layers |
| `<hash>_aa_coordinates.tsv` | `Chain, Residue, ResNum, X, Y, Z` for every residue |
| `<hash>_contacts_aa.txt` | `chain_from, aa_from, res_num_from, aa_to, res_num_to, chain_to` |
| `<hash>_all.zip` | the per-structure bundle offered for download |

**Upstream:** `https://huggingface.co/datasets/isalgo/vdjdb_structure_models`, fetched by
`tools/sync_structure_files.py`. On the server they live in
`~/vdjdb_publish/vdjdb-db/structure/` (118,348 files as of 2026-08-19).

**Generator:** `https://github.com/antigenomics/tcr-structures-visualization`, last upstream code
commit `946fdb6` (2026-05-22). Imported intact from the group's internal GitLab
(`gitlab.aldan3.itm-rsmu.ru/isagroup/tcr-structures-visualization`), which remains the place work
happens — the GitHub copy is a mirror, so check both before assuming a version. The pipeline entry
point is `produce_plots_pipline/run_pipeline.py`; `plotting.py` draws the map (PCA projection of the
Cα coordinates) and writes the two sidecar files. It is **not** installed on the vdjdb server.

⚠️ **The SVG carries no semantic markup.** matplotlib emits only its automatic element ids
(`line2d_N`, `text_N`, `patch_N`), so nothing in the file says which residue or which contact an
element is, and the ordinals do not line up with the contact list either (one sampled structure: 22
contact rows, 85 `line2d_*` elements). Anything in the viewer that needs to address a residue or a
contact line — hover highlighting, for one — requires the generator to `set_gid` on each artist
first, which matplotlib writes out as the SVG `id`. That is a change in the generator repo followed
by a re-render of all ~118k files, not something this repository can do.

---

## `app/backend/.../demo` — demo dataset

**Production:** `~/vdjdb_publish/demo/`, bind-mounted to `/home/vdjdb/demo-dataset/`, served by
`AnnotationsAPI` for the "Download an example sample" link.

These files live **only on the server** — there is no `demo/` in this repo and the bind mount reads
the host directory, not the image, so a redeploy neither adds nor removes them. `demoSamples` lists
the directory per request, so dropping a file in is enough to publish it: no restart, no code change.

| File | Format | Contents | Origin |
|---|---|---|---|
| `B35+.txt.gz` | VDJtools | TRB repertoire, HLA-B\*35 donor | predates this record; on the server since 2026-04-02 |
| `CMV+.txt.gz` | VDJtools | TRB repertoire, CMV-seropositive donor | predates this record; on the server since 2026-04-02 |
| `mixed.airr.tsv.gz` | AIRR | 100,000 rows, 50,000 TRA + 50,000 TRB; **synthetic** | `~/hf/airr_benchmark/test/airr/mixed.airr.tsv.gz`, copied 2026-07-22 |

`mixed.airr.tsv.gz` is there to demonstrate two things the other two cannot: the AIRR reader, and the
mixed-chain split that turns one uploaded file into a TRA sample and a TRB sample. Its rows are
generated, not measured — do not cite it as data. 1,869,266 B gzipped, 9,453,038 B raw (5.06x, well
inside the 100x decompression-ratio guard), md5 `49946bc42d33b4a35bf1a214b071e576`.

---

## `conf/control/` — enrichment null (control repertoires)

**What it is:** four healthy-repertoire samples, one per (species, chain). The annotation summary
searches the matching one through the *same* index and the *same* filters as the user's sample, and the
per-epitope match counts become the Beta prior the enrichment p-values are tested against.

**Provenance:** *experimental*, subsampled. The counts derived from them at runtime are *computed*.

| File | Clonotypes | Available upstream |
|---|---|---|
| `HomoSapiens.TRA.txt.gz` | 1,000,000 | 4,092,278 |
| `HomoSapiens.TRB.txt.gz` | 1,000,000 | 28,257,621 |
| `MusMusculus.TRA.txt.gz` | 627,648 | 627,648 (all) |
| `MusMusculus.TRB.txt.gz` | 1,000,000 | 1,177,591 |

**Upstream:** `https://huggingface.co/datasets/isalgo/airr_control` (locally `~/hf/airr_control/`), the
**`*.ntvj.vdjtools.tsv.gz`** files. VDJtools format, cut to the 11 positional columns.

### Two things about the sampling that are load-bearing

**`ntvj`, not `aa` — a clonotype must mean the same thing on both sides.** Uploaded samples are
nucleotide-level. The `aa` builds are collapsed to one row per `cdr3aa`, and convergent recombination
concentrates on exactly the germline-proximal public sequences VDJdb holds: measured on a real donor,
**1.63x** nt variants per aa sequence among VDJdb-matching clonotypes against **1.04x** overall. Against
an aa-collapsed control every uploaded sample looks uniformly enriched for that reason alone. Measured
as the ratio of overall VDJdb match rate, sample against control, on the CMV+ demo donor:

| control | exact | hamming-1 |
|---|---|---|
| `aa` (collapsed) | 18.7x | 7.0x |
| **`ntvj`** | **1.3x** | **1.07x** |

An offset near 1 is the point: it means the two repertoires are being counted the same way, and the
publicness correction in the test has almost nothing left to do.

**Uniform random, never `head -N`.** The files are pooled across many donors and sorted by abundance, so
the top rows are the most expanded and most shared sequences in the cohort.

**Productive only.** Rows whose `cdr3aa` contains `*` or `_` are dropped. Mouse TRA runs 21.3%
stop-codon and frameshift reads at the top of its count distribution; those can never match a database
of productive CDR3s, so leaving them in dilutes the denominator and returns a rate that is too low.

### Where they come from at build time

**Not committed.** Four gzipped repertoires are ~65 MB and this repository has never held a tracked
file above 40 KB. They live in the dataset they were derived from:

`https://huggingface.co/datasets/isalgo/airr_control/tree/main/vdjdb-web-control`

`tools/fetch_control.sh` pulls them into `conf/` (which is on the application classpath) and both CI
workflows run it before the build. It fails loudly on a bad download rather than leaving a partial
file, because an application missing its control starts normally and simply shows no p-values — there
is no symptom until somebody reads a chart.

```zsh
./tools/fetch_control.sh          # into conf/control
```

`ControlRepertoires` looks beside the database first (`<database.path>/control/`) and falls back to the
packaged copy, so a deployment can override them without a rebuild.

### Regenerate

Seeded, so it reproduces exactly. `tools/build_control.py`, seed `20260722`, size
`min(1_000_000, available)`:

```zsh
python3 tools/build_control.py conf/control
```

Unlike the table it replaces, this does **not** need regenerating when VDJdb is updated: the counts are
computed at request time against whatever database is loaded, so they cannot go stale. Regenerate only
to change the sampling or pick up a new release of `airr_control`.

